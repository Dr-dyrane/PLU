...
## Remaining lesson and catalogue alignment — 2026-09-07

The shared ready template now removes repeated form/color prose and the repeated product name in Code. Long exact forms have a wider phone cue, and reviewed-panel credits remain in an accessible Photo source disclosure. Relationship lessons use the same compact pacing, with all codes retained and full evidence in Listing / Related / Sources. Catalogue copy is shorter; filtered relationship/reference collections show a count instead of a nonfunctional expansion control.

Local browser checks covered 375 × 667 light/dark and 1440 × 900 desktop layouts. Aloe advanced through recognition into Code and Practice; Vanilla Case retained its exact label and full photo-source/reuse disclosure. Gala passed incomplete/duplicate rejection, reveal/retry, unordered full-set recall, wrong-checkout repair, completion, refresh and reset. Red Mango Case retained its unconfirmed handwritten neighbor and missing unit; White Yam passed single-code recall. Relationship sheets passed initial/repeated focus, keyboard tabs, Escape/backdrop/close dismissal, focus return, 44-pixel controls and viewport containment. Search, filter selection and Done navigation worked; console checks were clear. Canonical data and all mastery boundaries were unchanged.

## Lesson completion pass — 2026-09-07

Completion now offers a retry and the next catalog-ordered lesson or study. Ready lessons, relationship studies, and reference studies have separate sequences; the final item returns home through Done. Each product route keys its interactive component by identity, so a new item cannot inherit the previous item's answers or completion. Only one lightweight destination is passed to the client.

The ready lesson synchronously locks recognition feedback and the final practice transition. Restart and unmount cancel all owned timers; session fencing rejects already-queued callbacks. Exact recall and the existing storage boundaries are unchanged. Focused tests cover repeated input, timer cancellation, stale sessions, and all 463 unique routes across the three sequences.

Local rendered checks covered desktop dark/light, a 437 × 782 phone, and compact 375 × 667 dark layouts. Green Pepper completed all five steps: repeated choice taps advanced only one question, wrong recognition recovered, final practice taps were disabled, Recall began empty, keyboard error/repair rejected premature digits, and exact recall exposed Next lesson. Red Bell Pepper opened at Look and browser Back returned to a fresh Green Pepper exercise. Gala full-set recall, Study again, and Next study worked. Celery Hearts recorded-code recall continued to a fresh Goldendew study with its AI label intact. The final handwritten Walnuts study used source-status choices, restored after refresh, and Done returned home. Checked console output was clear.
