...
Batch 06 accounts for every source row left after Batch 05. Two rows have exact codes and human-reviewed checkout-recognition photos, bringing the ready lesson total to 302. The other 173 stay searchable but locked with a concrete source-review reason; no code, product identity, package claim, or image is invented to make the count look complete.
...
`media-resolution-batch04.json` records the corrected audit of products 101–200. `media-resolution-batch05.json` records every accepted and rejected Batch 05 candidate. `media-resolution-batch06.json` records the reviewed ready subset and the source-review disposition of the remaining catalog.
